...
const DATA = path.join(root, "Patients", "Johh Smith Jr");
...
function buildBpPatches() {
  // OURA-TASK SCOPE: Withings is a separate, non-Oura source. Per the ingestion
  // rules we must not touch it here (merging bp_list would even CREATE
  // withings_cuff rows for a patient whose Withings hasn't been ingested). A
  // dedicated Withings ingest owns that data. Always empty in this Oura loader.
  return new Map();
  /* eslint-disable no-unreachable */
  const byDay = new Map();
  const bpFile = path.join(DATA, "Withings", "bp.csv");
  if (!fs.existsSync(bpFile)) return byDay; // no Withings cuff for this patient
  for (const r of parseCSV(bpFile, ",")) {
    const ts = r.Date || "";
