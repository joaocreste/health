...
/* Runs IN the source page: FORCE every chart to an explicit width (the PDF card
   inner width) before capture. The source pages render at a narrow mobile width,
   so charts (Plotly AND Chart.js) draw too small for the full-width report cards;
   resizing to the container isn't enough because the source container is itself
   narrow. Forcing an explicit width re-renders them at report size regardless. */
async function fitCharts(targetW) {
  const W = targetW || 540;
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  try { window.dispatchEvent(new Event("resize")); } catch (e) { /* noop */ }
  await sleep(200);

  // Force WIDTH to the card width but KEEP each chart's original HEIGHT — scaling
  // height by the (narrow) aspect ratio would over-inflate it and the container's
  // overflow clip would cut the axis off the bottom.
  if (window.Plotly && window.Plotly.relayout) {
    for (const el of document.querySelectorAll(".js-plotly-plot")) {
      try {
        const fl = el._fullLayout || {};
        const h = fl.height || el.clientHeight || 320;
        el.style.width = W + "px";
        await window.Plotly.relayout(el, { width: W, height: h });
      } catch (e) { /* noop */ }
    }
  }

  if (window.Chart) {
    const charts = typeof window.Chart.getChart === "function"
      ? [...document.querySelectorAll("canvas")].map((cv) => window.Chart.getChart(cv)).filter(Boolean)
      : (window.Chart.instances ? Object.values(window.Chart.instances) : []);
    charts.forEach((c) => {
      try {
        const cv = c.canvas;
        const h = cv.clientHeight || 300;
        if (cv.parentNode) cv.parentNode.style.width = W + "px";
        c.resize(W, h);
        c.update("none");
      } catch (e) { /* noop */ }
    });
...
export async function buildReportPdf(env, args) {
  ...
  try {
    ...
    for (const pg of PAGE_ORDER) {
      ...
      await page.evaluate(fitCharts, 560);            // force charts to report-card width
