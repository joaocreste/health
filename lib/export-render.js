...
/* Runs IN the source page: re-layout every chart to fill its container width so
   it isn't captured at the narrow width it happened to draw at on load. Plotly
   charts otherwise keep their initial (often too-small) size; Chart.js the same. */
async function fitCharts() {
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  if (window.Plotly && window.Plotly.Plots) {
    for (const el of document.querySelectorAll(".js-plotly-plot")) {
      try {
        el.style.width = "100%";
        await window.Plotly.relayout(el, { autosize: true, width: null, height: null });
  ...
  if (window.Chart && typeof window.Chart.getChart === "function") {
    document.querySelectorAll("canvas").forEach((cv) => {
      try { const c = window.Chart.getChart(cv); if (c) c.resize(); } catch (e) { /* noop */ }
    });
  }
  await sleep(350);
}
...
export async function buildReportPdf(env, args) {
  ...
  try {
    ...
    for (const pg of PAGE_ORDER) {
      ...
      await page.evaluate(fitCharts);                 // re-size charts to fill their cards
