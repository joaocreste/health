...
/* Runs IN the page: snapshot <canvas> charts to <img> (so Chart.js survives the
   HTML copy), then return the outerHTML of the requested sections in DOM order. */
function extractSections(idList) {
  ...
  const out = [];
  document.querySelectorAll(".report-section, .sp-section").forEach((sec) => {
    if (sec.id && want.has(sec.id)) out.push(sec.outerHTML);
  });
  return out;
}
