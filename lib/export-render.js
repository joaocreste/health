  ...
  if (window.Plotly && window.Plotly.relayout) {
    for (const el of document.querySelectorAll(".js-plotly-plot")) {
      try {
        ...
        const w = fl.width || el.clientWidth || W;
        const ratio = h && w ? h / w : 0.62;
        el.style.width = W + "px";
        await window.Plotly.relayout(el, { width: W, height: Math.max(220, Math.round(W * ratio)) });
      } catch (e) { /* noop */ }
    }
  }
  ...
  if (window.Chart) {
    ...
    charts.forEach((c) => {
      try {
        ...
        const ratio = h && w ? h / w : 0.5;
        if (cv.parentNode) cv.parentNode.style.width = W + "px";
        c.resize(W, Math.max(200, Math.round(W * ratio)));
        c.update("none");
      } catch (e) { /* noop */ }
    });
  }
