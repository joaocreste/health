...
Return the doctor's name (with title, e.g. "Dr. João Silva" or "Dra. Maria Souza"). If not present, return null. Do not invent.
    ...
    const rows = await sql`
      ...
      VALUES
        (${patientId}, ${m.panel || null}, ${m.marker}, ${m.value ?? null}, ${m.value_text || null},
         ...
         ${taken}::date, ${extracted.laboratory || null}, ${extracted.requesting_doctor || null}, ${blobKey})
      RETURNING id
    `;
    ids.push(rows[0].id);
  }
...
Return the full name with title (Dr./Dra.). If absent, null. Do not invent.`;
  ...
  for (const r of rows) {
    ...
    try {
      ...
      if (doctor) {
        ...
        result.note = "no requesting doctor found in PDF";
      }
    } catch (e) {
