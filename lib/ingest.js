...
COMPLETENESS — read every page, in whatever layout the report uses: tables, multi-column grids, OR free-text blocks where each analyte is a labeled paragraph ("NAME ... Resultado: <value> <unit>"). A value in prose counts as much as one in a table cell. Capture the ENTIRE urinalysis section (every qualitative dipstick/sediment line) as value_text — those are the most-dropped lines. One entry per analyte per collection date.

Do not invent values. Omit a marker ONLY if it is truly illegible or absent.`;

async function extractLabResults(client, pdfBase64) {
  const resp = await client.messages.create({
    model: "claude-sonnet-4-6",
    max_tokens: 16000, // large full panels (80+ analytes) exceed 4000 output tokens and would truncate mid-JSON
...
Step 2: if class == "lab_pdf", extract EVERY lab result in the PDF — read every page, in whatever layout it is printed.
  - Results appear as tables, multi-column grids, OR free-text blocks where each analyte is its own labeled paragraph (a NAME header, then "Resultado: <value> <unit>", then "Valor de referência: ..."). A value printed in prose counts EXACTLY as much as one in a table cell — never skip a result because it is not in a tidy table.
  - Capture the ENTIRE urinalysis section when present: every qualitative dipstick and sediment line (color, aspect/appearance, density, pH, glucose, protein, nitrite, ketones, bilirubin, urobilinogen, blood/hemoglobin, leukocytes, erythrocytes, epithelial cells, crystals, casts, yeast, mucus, bacteria) as value_text. These qualitative lines are the ones most often dropped — do NOT drop them.
  - One entry per analyte per collection date: if the same marker is printed on two pages, include it once.
  - Before finishing, scan every page and confirm you emitted a result for each analyte block you saw. If your list looks short for the number of pages, re-read the page you skipped.
...
- Never invent values. Omit a marker ONLY if it is truly illegible or absent — prefer capturing a qualitative result as value_text over dropping the line.
...
async function classifyAndExtractPdf(client, file) {
  ...
  const resp = await client.messages.create({
    model: "claude-sonnet-4-6",
    max_tokens: 16000, // large full panels (80+ analytes) exceed 8000 output tokens and would truncate mid-JSON
