...
- When ANY domain has data, summary.headline and summary.overview {"en","pt"}
  are REQUIRED (2-4 sentences for the overview). When no domain has data, they
  stay empty.
...
export function computeInputCoverage(record) {
  ...
  const physical = !!(
    ...
    (r.labs?.total_results > 0) || (r.vitals?.summary?.days > 0) ||
    (r.glucose?.points > 0) || some(r.imaging_studies) ||
    some(r.ergometric_studies) || some(r.electrodiagnostic_studies) ||
  ...
  return { physical, mental, spiritual, any: physical || mental || spiritual };
}
...
export function renderInputCoverage(cov) {
  ...
  return [
    ...
    cov.any
      ? "- summary: headline + overview {en,pt} REQUIRED (some domain has data)"
