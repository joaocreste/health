...
function sanitizeSourceCoverage(raw, domainsPresent) {
  const r = raw && typeof raw === "object" ? raw : {};
  return {
    database: r.database === true,
    files_reports: r.files_reports === true,
    curated_frontend: r.curated_frontend === true,
    discussed_with_patient: r.discussed_with_patient === true,
    domains_present: Array.isArray(r.domains_present)
      ? r.domains_present.filter((d) => PAGES.has(d))
      : domainsPresent,
    notes: bilingOpt(r.notes) || { en: "", pt: "" },
  };
}

function sanitizePayload(parsed, { patientId, patientName, generatedAt, version }) {
  ...
  return {
    ...
    source_coverage: sanitizeSourceCoverage(parsed?.source_coverage, domainsPresent),
    pages: {
      physical: {
...
export async function rebuildAiInsights({
  ...
  const payload = sanitizePayload(parsed, { patientId, patientName: displayName, generatedAt, version });
