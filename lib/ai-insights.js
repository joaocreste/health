 ...
 *
 * Distinct from lib/dashboard.js (per-section card generator). This is
 * the "complete rewrite of every AI insight" authoring pass:
 *
 *   1. Assemble the patient's ENTIRE structured record from Neon.
 *   2. Run claude-opus-4-8 (adaptive thinking, effort high) with the strict
...
// claude-opus-4-8: the patient-facing synthesis runs on Opus for top quality
// (platform-wide "4.8 for everything" decision, 2026-07-21). Opus 4.8 is
// $5/$25 per MTok — the earlier claude-sonnet-5 pass traded some quality for
// ~60% lower cost; here we buy the quality back. High-volume document
// extraction (lib/ingest.js) deliberately stays on Haiku/Sonnet.
import { assignRanks, joinBiling } from "./card-order.js";
 ...
 * template keeps its five placeholders literal — {{PATIENT_NAME}}, {{CURRENT_DATE}},
 * {{INSIGHTS_VERSION}}, {{INPUT_COVERAGE}}, {{PATIENT_RECORD}} — and they are
 * interpolated at call time by fillUserPrompt(). Everything in the block comments
 * below is human-facing documentation and is NOT sent to the model.
 *
 ...
 * Interpolate ALL five placeholders before sending. Sending it with placeholders
 ...
 *   {{INPUT_COVERAGE}}    → the deterministic per-domain coverage manifest
 *                           (computeInputCoverage + renderInputCoverage)
 ...
 * USER_PROMPT_TEMPLATE as the user message; model claude-opus-4-8, adaptive
...
- If the input_coverage manifest in the user message marks EVERY domain NO DATA
  (or the record below is missing or still in template form): return a VALID
  object for this patient with empty attention_points/strengths arrays
  everywhere, every data_sufficient and data_available set to false, an empty
  summary, and empty inline_insights. Do NOT fabricate any health data. Do NOT
  ask what was intended. Never apply this rule to a domain the manifest marks
  HAS DATA — however thin the record looks, the manifest is authoritative.
...
- Patients vary enormously in how much data they have. Never fabricate to fill a
  ...
  none). Honesty over completeness at the curated layer; exhaustiveness only over
  data that actually exists.

PER-DOMAIN COVERAGE CONTRACT — AUTHORITATIVE (prompt #2c guarantee)
The user message carries an input_coverage manifest computed DETERMINISTICALLY
from the assembled record. It is authoritative — do not second-guess it. The
build is rejected and retried if you violate it.
- Domain marked HAS DATA: a concise overview {"en","pt"} of 2-4 sentences is
  REQUIRED, with BOTH languages written natively by you (never one copied into
  the other), and data_sufficient MUST be true. Even when the data supports no
  curated attention points or strengths, the overview still honestly describes
  what the record holds (what kinds of data, how much, over what period, and the
  overall picture). The overview never fabricates; it summarizes what exists.
- Domain marked NO DATA: there is nothing to infer over — leave it blank.
  data_sufficient MUST be false (and for spiritual, data_available false), the
  attention_points/strengths arrays MUST be empty, and there MUST be no overview
  text in either language.
- When ANY domain has data, summary.headline and summary.overview {"en","pt"}
  are REQUIRED. When no domain has data, they stay empty.

THE DEPTH LADDER — EVERY LEVEL READS DIFFERENTLY
The overviews form a ladder from glance to deep dive. Each level is written
fresh for its altitude and page; NEVER reuse or lightly rephrase the same
sentences across levels:
- summary.headline + summary.overview (Home): the MOST concise — 1 sentence
  headline, 1-2 sentence overview of the whole person.
- pages[domain].overview (Physical / Mental / Spiritual pages): 2-4 sentences
  framing that pillar — what kinds of data exist, the overall picture, the one
  or two things that matter most.
- page_overviews[subpage] (Vitals / Exams / Genetics pages): the deepest —
  3-6 sentences covering THAT page's data specifically (what is on file, over
  what period, the key findings and trends on that page), for a reader who
  navigated there to dig in. Required for every subpage the manifest marks
  HAS DATA; omit the key entirely for subpages marked NO DATA.
...

{
  ...
  "pages": {
    "physical": {
      "data_sufficient": true,                   // true iff input_coverage marks this domain HAS DATA
      "overview": { "en": "", "pt": "" },        // 2-4 sentences framing this page;
                                                 // REQUIRED when the domain has input data,
  ...
  "page_overviews": {
    // Per-SUBPAGE deep overviews (3-6 sentences each) — REQUIRED for every
    // subpage the input_coverage manifest marks HAS DATA, omitted otherwise.
    // Each digs into THAT page's specific data only, deeper than the pillar
    // overview and never repeating its sentences:
    "physical-vitals":   { "en": "", "pt": "" },  // wearables, daily vitals, glucose, ECG events
    "physical-exams":    { "en": "", "pt": "" },  // labs, imaging, sleep/ENMG/ergometric studies, reports
    "physical-genetics": { "en": "", "pt": "" }   // pharmacogenomic findings and their drug interactions
  },

  "summary": {
    "headline": { "en": "", "pt": "" },         // 1 sentence overall picture, as of generated_at
    "overview": { "en": "", "pt": "" },         // 1-2 sentences — the MOST concise text of the build
 ...
 * The five {{...}} placeholders are kept LITERAL here on purpose. They are
...
input_coverage (authoritative — computed deterministically from the record;
the per-domain coverage contract in the system message binds you to it):
{{INPUT_COVERAGE}}
...
// Interpolate the five canonical placeholders at call time. Replaces every
// occurrence (PATIENT_NAME appears twice). Keep the template's {{...}} literal.
function fillUserPrompt({ patientName, currentDate, insightsVersion, inputCoverage, patientRecord }) {
  return USER_PROMPT_TEMPLATE
    .replaceAll("{{PATIENT_NAME}}", patientName ?? "")
    .replaceAll("{{CURRENT_DATE}}", currentDate ?? "")
    .replaceAll("{{INSIGHTS_VERSION}}", String(insightsVersion ?? ""))
    .replaceAll("{{INPUT_COVERAGE}}", inputCoverage ?? "")
...
export async function assembleRecord(sql, patientId) {
  const [
    ...
    imaging, ergometric, edx, sleep, ecg, ecgStudies, pgx, writings, moodAgg, moodRecent, panic, life, psych, wheel, reflective, docs,
  ] = await Promise.all([
    ...
    // Clinical ECG studies (12-lead / rhythm, migration 0012) — distinct from the
    // wearable ecg_events above. The AI reads the full record; validating_doctor
    // NULL means the reading is an unsigned/automated interpretation, not a
    // clinician-validated finding — the model must weight it accordingly.
    sql`SELECT study_date, modality, lead_layout, ordering_doctor, validating_doctor,
               clinic, heart_rate, pr_ms, qrs_ms, qt_ms, qtc_ms, axis_p, axis_qrs, axis_t,
               fidelity, interpretation, report_text
        FROM ecg_studies WHERE patient_id = ${patientId}
        ORDER BY study_date DESC NULLS LAST`,
...
/* ───── Input coverage (prompt #2c guarantee) ───────────────────────
 * Which domains have ANY ingested data, computed DETERMINISTICALLY from the
 * assembled record — the same object the model receives. This single rule
 * drives (a) the {{INPUT_COVERAGE}} manifest in the prompt, (b) the coverage
 * assertion that rejects non-compliant builds, and (c) the stored
 * data_sufficient flags. No data → nothing to infer on → blank. */
/* Documents feed the model (classifier summaries) and are the ONLY rows a
 * freshly-ingested upload may have — they must count as input data or the
 * manifest forbids summarizing content the prompt orders the model to read. */
const DOC_MENTAL_KINDS = new Set(["writing", "journal", "therapy_session", "psych_report"]);

export const SUBPAGES = ["physical-vitals", "physical-exams", "physical-genetics"];

export function computeInputCoverage(record) {
  const r = record && typeof record === "object" ? record : {};
  const some = (a) => Array.isArray(a) && a.length > 0;
  const docs = Array.isArray(r.documents) ? r.documents : [];
  /* Per-subpage coverage: each physical subpage renders its own deeper
     overview, so its data presence is tracked separately. */
  const vitalsPage = !!(
    (r.vitals?.summary?.days > 0) || (r.glucose?.points > 0) || some(r.ecg_events)
  );
  const examsPage = !!(
    (r.labs?.total_results > 0) || some(r.imaging_studies) ||
    some(r.sleep_studies) || some(r.electrodiagnostic_studies) ||
    some(r.ergometric_studies) || some(r.ecg_studies) ||
    docs.some((d) => d && !DOC_MENTAL_KINDS.has(d.kind))
  );
  const geneticsPage = some(r.pgx_findings);
  const physical = !!(
    vitalsPage || examsPage || geneticsPage ||
    some(r.medications) || some(r.supplements) || some(r.surgeries) ||
    some(r.injuries) || some(r.clinical_history) ||
    (r.profile && (r.profile.height_cm != null || r.profile.weight_kg != null || r.profile.blood_type))
  );
  const mental = !!(
    some(r.psych_architecture) || (r.mood?.summary?.n > 0) ||
    some(r.panic_events) || some(r.risk_assessments) || some(r.writings?.recent) ||
    docs.some((d) => d && DOC_MENTAL_KINDS.has(d.kind))
  );
  const spiritual = !!(
    some(r.wheel_of_life) || some(r.life_events) || some(r.reflective_items)
  );
  return {
    physical, mental, spiritual,
    any: physical || mental || spiritual,
    pages: {
      "physical-vitals": vitalsPage,
      "physical-exams": examsPage,
      "physical-genetics": geneticsPage,
    },
  };
}

export function renderInputCoverage(cov) {
  const line = (name, has) => has
    ? `- ${name}: HAS DATA -> overview {en,pt} (2-4 sentences, both languages native) REQUIRED; data_sufficient: true`
    : `- ${name}: NO DATA -> data_sufficient: false, empty attention_points/strengths, NO overview text`;
  const sub = (name, has) => has
    ? `- ${name} (page_overviews): HAS DATA -> a DEEPER page-specific overview {en,pt} (3-6 sentences) REQUIRED`
    : `- ${name} (page_overviews): NO DATA -> omit this key entirely, no text`;
  const p = cov.pages || {};
  return [
    line("physical", cov.physical),
    sub("physical-vitals", p["physical-vitals"]),
    sub("physical-exams", p["physical-exams"]),
    sub("physical-genetics", p["physical-genetics"]),
    line("mental", cov.mental),
    line("spiritual", cov.spiritual) + (cov.spiritual ? "" : "; data_available: false"),
    cov.any
      ? "- summary: headline (1 sentence) + overview (1-2 sentences, the MOST concise text of the build) {en,pt} REQUIRED"
      : "- summary: headline and overview stay EMPTY (no domain has data)",
  ].join("\n");
}

/* Strict bilingual check for REQUIRED summary objects: both languages must be
 * present as written by the model — no en||pt backfill masking a missing side. */
function strictBiling(v) {
  return !!(v && typeof v === "object" &&
    typeof v.en === "string" && v.en.trim() &&
    typeof v.pt === "string" && v.pt.trim());
}
function anyText(v) {
  // Mirror biling()'s string branch: a plain-string overview would be
  // normalized into stored {en,pt} content downstream, so it must count as
  // text here or a string slips past the zero-data fabrication rejection.
  if (typeof v === "string") return !!v.trim();
  return !!(v && typeof v === "object" &&
    ((typeof v.en === "string" && v.en.trim()) || (typeof v.pt === "string" && v.pt.trim())));
}

/* Reject a parsed build that violates the coverage contract. Called inside the
 * retry loop (a violation earns the model a second attempt) and again from
 * sanitizePayload as defense in depth — nothing partial is ever stored. */
export function assertCoverage(parsed, record) {
  if (!record) return; // no record context (legacy callers) — nothing to assert against
  const cov = computeInputCoverage(record);
  const pages = parsed?.pages && typeof parsed.pages === "object" ? parsed.pages : {};
  for (const name of ["physical", "mental", "spiritual"]) {
    const pg = pages[name] && typeof pages[name] === "object" ? pages[name] : {};
    const items = (Array.isArray(pg.attention_points) ? pg.attention_points.length : 0) +
      (Array.isArray(pg.strengths) ? pg.strengths.length : 0);
    if (cov[name]) {
      if (!strictBiling(pg.overview)) {
        throw new Error(`ai_insights_coverage_violation: ${name} has input data but the build lacks a bilingual overview (both en and pt required)`);
      }
    } else if (anyText(pg.overview) || items > 0) {
      throw new Error(`ai_insights_coverage_violation: ${name} has no input data but the build contains summary content — nothing to infer on, must stay blank`);
    }
  }
  /* Per-subpage overviews: required where the page has data, forbidden where
     it has none (same rule, page granularity). */
  const po = parsed?.page_overviews && typeof parsed.page_overviews === "object" ? parsed.page_overviews : {};
  for (const pg of SUBPAGES) {
    if (cov.pages[pg]) {
      if (!strictBiling(po[pg])) {
        throw new Error(`ai_insights_coverage_violation: ${pg} has input data but page_overviews lacks a bilingual entry for it`);
      }
    } else if (anyText(po[pg])) {
      throw new Error(`ai_insights_coverage_violation: ${pg} has no input data but page_overviews contains text for it`);
    }
  }
  const summary = parsed?.summary && typeof parsed.summary === "object" ? parsed.summary : {};
  if (cov.any) {
    if (!strictBiling(summary.headline) || !strictBiling(summary.overview)) {
      throw new Error("ai_insights_coverage_violation: record has data but summary.headline/overview is not fully bilingual");
    }
  } else if (anyText(summary.headline) || anyText(summary.overview)) {
    throw new Error("ai_insights_coverage_violation: empty record but the build contains summary text");
  }
}
...
function sanitizeInline(raw) {
  ...
  return raw.slice(0, 120).map((x, i) => {
    // Contract §3: a legacy {kind,title,subtitle} card in a NEW build is a
    // sanitize error — the async build must emit canonical cards only.
    if (x && typeof x === "object" && "kind" in x && !x.trigger) {
      throw new Error(
        `legacy card shape ({kind:"${x.kind}"}) at inline_insights[${i}] — ` +
        "new builds must emit canonical cards only (contract section 3)");
    }
    ...
    // Contract §3: body / plain_language_reading / what_the_report_says are
    // deprecated and no longer emitted — their content folds into the
    // canonical `interpretation` channel (imaging joins its two channels as
    // separate paragraphs; body is the fallback for vitals/journal/pgx).
    const foldedInterpretation = isLab
      ? interpretation
      : isImaging
        ? (joinBiling(plain_language_reading, what_the_report_says) || interpretation || body)
        : (interpretation || body);
    return {
      ...
      interpretation: foldedInterpretation,
...
function sanitizePayload(parsed, { patientId, patientName, generatedAt, version, record = null }) {
  // Prompt #2c guarantee, defense in depth: a payload violating the coverage
  // contract must never be stored, whichever path reached us.
  assertCoverage(parsed, record);
  const cov = record ? computeInputCoverage(record) : null;
  ...
  const spiritualHasData = cov ? cov.spiritual : (
  ...
  // domains_present: input-derived when the record is available (the coverage
  // rule); output-derived fallback for legacy callers without a record.
  const domainsPresent = [];
  if (cov ? cov.physical : physical.attention.length + physical.strengths.length > 0) domainsPresent.push("physical");
  if (cov ? cov.mental : mental.attention.length + mental.strengths.length > 0) domainsPresent.push("mental");
  ...
  return {
    ...
    pages: {
      // data_sufficient is the input-coverage verdict when the record is
      // available (assertCoverage already guaranteed the overview exists for
      // data-bearing domains): data present -> true, none -> false. The
      // output-derived item-count rule remains only for legacy record-less
      // callers.
      physical: {
        data_sufficient: cov ? cov.physical :
          (pagesRaw.physical?.data_sufficient !== false &&
      ...
      mental: {
        data_sufficient: cov ? cov.mental :
          (pagesRaw.mental?.data_sufficient !== false &&
      ...
      spiritual: {
        ...
        data_sufficient: cov ? cov.spiritual :
          (spiritualHasData &&
    ...
    /* Per-subpage deep overviews (depth ladder). With a record present, only
       data-bearing subpages persist (assertCoverage already guaranteed their
       presence and rejected text on empty ones); legacy record-less callers
       keep whatever sanitizes. */
    page_overviews: (() => {
      const raw = parsed?.page_overviews && typeof parsed.page_overviews === "object" ? parsed.page_overviews : {};
      const keep = cov ? SUBPAGES.filter((p) => cov.pages[p]) : SUBPAGES;
      const out = {};
      for (const p of keep) { const b = biling(raw[p]); if (b) out[p] = b; }
      return out;
    })(),
    summary: {
    ...
    // D1: deterministic order — sorted by cardSortKey, rank persisted 0..n-1.
    inline_insights: assignRanks(sanitizeInline(parsed?.inline_insights)),
  };
}
...
// Shared request shape for the live (streaming) and batch paths. Cache notes:
// - The OUTPUT_SCHEMA breakpoint caches SYSTEM_PROMPT + schema (the shared
//   prefix across ALL patients) — this is the one that hits across runs.
// - The user-message breakpoint only serves the immediate in-run retry:
//   generated_at and insights_version interpolate ABOVE the record, so the
//   prefix differs on every rebuild anyway. Default 5-minute TTL (1.25x write)
//   covers the retry; the old 1h TTL (2x write) bought nothing extra.
function buildModelParams({ record, displayName, currentDate, version }) {
  const userText = fillUserPrompt({
    patientName: displayName,
    currentDate,
    insightsVersion: version,
    inputCoverage: renderInputCoverage(computeInputCoverage(record)),
  ...
  return {
    ...
    // truncating the JSON mid-array. 128k = the model maximum (the call
    // already streams): Paulo hit 64k and Leo hit 96k exactly, each losing
    // trailing inline cards to truncation repair.
    max_tokens: 128000,
    ...
    messages: [
      {
        role: "user",
        content: [{ type: "text", text: userText, cache_control: { type: "ephemeral" } }],
      },
    ],
  };
}
...
async function callModel(anthropic, { record, displayName, currentDate, version }, onTick) {
  const stream = anthropic.messages.stream(
    buildModelParams({ record, displayName, currentDate, version }),
  );
...
// Parse the model's raw text and enforce the single-patient contract.
// Throws on any violation — callers decide whether to retry.
function parseValidated(text, displayName) {
...
// Sanitize + upsert one validated payload. Shared by the live and batch paths.
async function persistInsights({
  sql, patientId, viewerId, parsed, record, displayName, generatedAt, version, usage,
}) {
  ...
  // Fingerprint: record which source-state this narrative reflects, so the read-time
  // guard (computeStale) can tell when newer data has been ingested since. Null => the
  // patient has no watermark row yet; COALESCE to now() (fresh as of this build).
  const wmRow = await sql`SELECT watermark FROM patient_source_watermark WHERE patient_id = ${patientId} LIMIT 1`;
  const builtAgainst = wmRow[0]?.watermark || null;

  await sql`
    INSERT INTO patient_dashboards
      (patient_id, section, summary_md, cards_json, model,
       input_tokens, output_tokens, generated_by, generated_at, built_against_watermark)
    VALUES
      (${patientId}, ${AI_INSIGHTS_SECTION}, ${summaryMd}, ${JSON.stringify(payload)}, ${MODEL},
       ${usage.input}, ${usage.output}, ${viewerId}, now(), COALESCE(${builtAgainst}, now()))
    ON CONFLICT (patient_id, section) DO UPDATE SET
      ...
      generated_at  = now(),
      built_against_watermark = EXCLUDED.built_against_watermark
  `;
...
export async function rebuildAiInsights({
  ...
  let record = await assembleRecord(sql, patientId);
  // Optional de-identification boundary (default off → no change for the Worker /
  // standard local runner). When provided, it scrubs the assembled record BEFORE
  // anything reaches the model — used to run rebuilds on real PHI while no BAA is
  // in place. displayName is taken from the (possibly tokenized) record below, so
  // the model never sees the real identifier.
  if (typeof deidentify === "function") record = deidentify(record) || record;
  ...
  // (very rarely) an array / wrong patient_name, or (prompt #2c) a payload that
  // violates the input-coverage contract. Reject and retry on any of those.
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    usage = res.usage;
    try {
      parsed = parseValidated(res.text, displayName);
      assertCoverage(parsed, record);
      break;
    } catch (e) {
      lastErr = e;
  ...
  if (!parsed) {
    const msg = lastErr?.message || "no JSON";
    // Keep the coverage-violation error name intact — it is a contract
    // rejection (clear, structural), not a parse failure.
    throw new Error(msg.startsWith("ai_insights_coverage_violation")
      ? msg : `ai_insights_unparseable: ${msg}`);
  }

  return persistInsights({
    sql, patientId, viewerId, parsed, record, displayName, generatedAt, version, usage,
  });
}

/**
 * Batch-mode rebuild for one or more patients via the Message Batches API —
 * 50% off ALL token usage vs the live path, at the cost of asynchrony (most
 * batches land within the hour; hard ceiling 24h). Same assemble -> prompt ->
 * validate -> persist pipeline as rebuildAiInsights; only transport differs.
 *
 * Failure semantics: per-patient. A patient whose result errors or fails
 * validation gets { ok: false, error } in the outcomes and their previous
 * insights stay untouched; rerun those via the live path. There is no
 * in-batch retry (each retry would need a new batch round-trip — the caller
 * decides whether the 50% discount is worth a second wait).
 */
export async function rebuildAiInsightsBatch({
  sql, anthropic, patientIds, viewerId = null, currentDate,
  deidentify = null, pollIntervalMs = 60_000, onStatus = null,
}) {
  const generatedAt = currentDate || new Date().toISOString();

  // 1. Assemble every record up front (DB work, no model cost).
  const jobs = [];
  for (const patientId of patientIds) {
    const version = await nextVersion(sql, patientId, null);
    let record = await assembleRecord(sql, patientId);
    if (typeof deidentify === "function") record = deidentify(record) || record;
    const displayName = record.profile.name || "the patient";
    jobs.push({ patientId, version, record, displayName, customId: `insights-${patientId}` });
  }

  // 2. One batch, one request per patient.
  const batch = await anthropic.messages.batches.create({
    requests: jobs.map((j) => ({
      custom_id: j.customId,
      params: buildModelParams({
        record: j.record, displayName: j.displayName,
        currentDate: generatedAt.slice(0, 10), version: j.version,
      }),
    })),
  });
  onStatus?.({ phase: "submitted", batchId: batch.id, count: jobs.length });
  ...
  while (b.processing_status !== "ended") {
    ...
    onStatus?.({ phase: "polling", batchId: batch.id, status: b.processing_status, counts: b.request_counts });
  }

  // 4. Persist results — arrive in ANY order; match by custom_id, never position.
  const byId = new Map(jobs.map((j) => [j.customId, j]));
  const outcomes = [];
  for await (const result of await anthropic.messages.batches.results(batch.id)) {
    const job = byId.get(result.custom_id);
    if (!job) continue;
    if (result.result.type !== "succeeded") {
      outcomes.push({ patientId: job.patientId, ok: false, error: `batch_result_${result.result.type}` });
      continue;
    }
    ...
    try {
      const parsed = parseValidated(text, job.displayName);
      assertCoverage(parsed, job.record); // no in-batch retry: violation -> ok:false, rerun live
      const persisted = await persistInsights({
        sql, patientId: job.patientId, viewerId, parsed,
        record: job.record, displayName: job.displayName,
        generatedAt, version: job.version, usage,
      });
      outcomes.push({ patientId: job.patientId, ok: true, ...persisted });
    } catch (e) {
      outcomes.push({ patientId: job.patientId, ok: false, error: e.message, usage });
    }
  }
  return { batchId: batch.id, outcomes };
}
