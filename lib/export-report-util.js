...
<style>
  ...
  * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  .report-section, .sp-section, .imagery-exam {
  ...
  /* Charts: hide Plotly's interactive mode-bar and clip any SVG bleed past the card. */
  .modebar, .modebar-container { display: none !important; }
  .chart-card, .chart-wrap, .js-plotly-plot, .plot-container, .svg-container { overflow: hidden !important; }
  /* Interactive widgets can't render in a static PDF — drop them, keep the
     description + radiologist's report. (The .ct-viewer is a live DICOM scrubber
     that otherwise prints as a blank black box.) */
  .ct-viewer { display: none !important; }
  select, input[type="range"], input[type="number"] { display: none !important; }

  /* Never split a card / chart / table across a page. */
  .metric-card, .lab-test, .lab-card, .ai-card, .ai-insight, .chart-wrap, .chart-card,
  .alert, table, figure, .panel, .imagery-exam { break-inside: avoid !important; page-break-inside: avoid !important; }
