...
| 21 | Identity data quality: duplicate "Patient Zero" empty shell vs `pending:joao`; Paulo DOB "—" vs "14 Jul 1961"; Leo "Paris, United Kingdom"; stale tab title | 2b note; 2a #7/#10; 2c D-1x+6 | Duplicate records; inconsistent sources | D7 cleanup + I-3 — 2026-07-10 (Prompt #2b): banner renders name/DOB/Locale/Prepared strictly from the API and tab titles now localize with real identity on all paths (I-3 wired); residual is data, not wiring: `country_of_residence` is one free-text string rendered identically in EN and PT (PT viewers see "London, United Kingdom" — localization needs structured residence data), and bad stored values (Leo "Paris, United Kingdom") now surface verbatim — D7 cleanup still required |
| 22 | **RESOLVED 2026-07-10 (Prompt #2b, prod-verified).** Static "prepared" dates diverge (24 Jun / 25 Apr / 4 May) | Pass 1 #9; code Job 2D | Hardcoded strings | Build prompt #2b: unified page banner — "Prepared" = newest dashboard `generated_at` via `renderPageBanner` (assembler slot 1 + static-trio swap in `decorateStaticBanner`); omitted when no dashboard exists; the three divergent hardcoded strings stripped from home/physical/spiritual shells |
...
| 30 | Spiritual identity chips (Tradition / Bible · KJV) retired with `sp-hero` in Prompt #2b — evaluate relocation into a spiritual content section in prompt #3 | 2b banner swap (design has no per-pillar identity chips; the information now survives only narratively in the page body) |
