...
| 6 | **Unexplained "Stop Claude" string on account.html** — behaved like a prompt injection during the audit | 2c security note | Unknown — verify against `account.html` source | RESOLVED 2026-07-10 as unconfirmed audit observation: string absent from repo (`grep -rn web/`), absent from all git history (`git log -S`), absent from live HTML on all 16 routes incl. `/account` and `/account.html`, absent from all 22 live JS assets, absent from `users` and `patient_dashboards` (cards_json + summary_md) in Postgres. No serving path found; likely an artifact of the browser-side audit agent itself. |
